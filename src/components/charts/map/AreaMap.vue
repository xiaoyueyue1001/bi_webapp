...
<script>
import ZcmorefunMap from "./Map.vue";
export default {
  name: "ZcmorefunAreaMap",
  components: {
    ZcmorefunMap
  },
  props: {
      ...
      this.option = {
        tooltip: {
          ...
          formatter: (params, ticket, callback) => {
            const color =
              "rgb(" +
              this.getColor(this.filteredData[params.dataIndex].value).join(
                ","
              ) +
              ")";
        ...
        visualMap: {
          ...
          inRange: {
            color: ["rgb(0, 186, 1)", "rgb(247, 221, 5)", "rgb(212, 63, 57)"]
          },
          textStyle: {
            color: "#bbbbbb"
          },
